...
void BrowserServer::init (int socketFd) { s_instance = std::make_unique<BrowserServer> (socketFd); }

void BrowserServer::shutdown () { s_instance.reset (); }

BrowserServer& BrowserServer::get () { return *s_instance; }
...
void BrowserServer::receiveLoop () {
	...
	try {
		...
		case MessageType::CreateBrowser:
		    handleCreateBrowser (r);
		    break;
		case MessageType::DestroyBrowser:
		    handleDestroyBrowser (r);
		    break;
		...
		case MessageType::Resize:
		    handleResize (r);
		    break;
		case MessageType::AssetResponse:
...
void BrowserServer::handleMouseButton (MessageReader& r) {
    ...
    CefPostTask (TID_UI, makeCefTask ([=, this] () {
		     ...
		     const auto btnType = (button == 0) ? CefBrowserHost::MouseButtonType::MBT_LEFT
