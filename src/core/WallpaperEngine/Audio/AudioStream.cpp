...
int audio_read_thread (void* arg) {
	...
	while (ret >= 0 && stream->getAudioContext ().getDriver ().getContext ().isRunning && stream->isInitialized ()) {
...
AudioStream::AudioStream (AudioContext& context, const Data::Utils::ReadStreamSharedPtr& buffer) :
