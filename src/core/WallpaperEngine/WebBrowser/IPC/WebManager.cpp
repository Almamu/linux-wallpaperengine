    ...
    while (m_running) {
        ...
        try {
            switch (static_cast<MessageType> (type)) {
                case MessageType::FrameReady:   handleFrameReady (r);   break;
                case MessageType::AssetRequest: handleAssetRequest (r); break;
                default:
...
void WebManager::createBrowser (
    const std::string& uuid, const std::string& url, int width, int height, int fps
) {
    std::lock_guard lock (m_framesMutex);
...
void WebManager::sendMouseButton (
    const std::string& uuid, float x, float y, uint8_t button, bool isRelease
) {
    MessageWriter w;
    w.writeString (uuid);
    w.writeFloat (x);
...
void WebManager::registerAssetLocator (
    const std::string& uuid, const Assets::AssetLocator* locator
) {
    std::lock_guard lock (m_assetMutex);
