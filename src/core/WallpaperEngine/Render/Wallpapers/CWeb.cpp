...
CWeb::CWeb (
    ...
    sWebManager.createBrowser (
        this->m_uuid, htmlURL, this->getWidth (), this->getHeight (),
        context.getContext ().config.web_fps
    );
}
...
void CWeb::updateMouse () {
    ...
    const auto buttonStatus = handler != nullptr && handler->is_pressed != nullptr ? handler->is_pressed (
        handler->user_parameter, WP_MOUSE_INPUT_BUTTON_LEFT | WP_MOUSE_INPUT_BUTTON_RIGHT
    ) : 0;
    ...
    if (leftStatus != this->m_leftClick) {
        sWebManager.sendMouseButton (
            ...
            0, leftStatus == WallpaperEngine::Input::MouseClickStatus::Released
        );
    ...
    if (rightStatus != this->m_rightClick) {
        sWebManager.sendMouseButton (
            this->m_uuid,
            static_cast<float> (position.x), static_cast<float> (position.y),
            1, rightStatus == WallpaperEngine::Input::MouseClickStatus::Released
        );
