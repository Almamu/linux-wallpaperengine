...
#define WPENGINE_PROJECT_API_END(result) } catch (...) { \
return result; \
}

wp_project* wp_project_load_id (wp_context* context, wp_mouse_input* mouse_input, const int id) {
	WPENGINE_PROJECT_API_BEGIN
	return WallpaperEngine::Project::loadId (
		static_cast<WallpaperEngine::Context*> (context), mouse_input, id
	);
...
wp_project* wp_project_load_folder (wp_context* context, wp_mouse_input* mouse_input, const char* folder) {
	WPENGINE_PROJECT_API_BEGIN
	return WallpaperEngine::Project::loadFolder(
		static_cast<WallpaperEngine::Context*> (context), mouse_input, folder
	);
	WPENGINE_PROJECT_API_END(nullptr);
}
...
void wp_project_set_output_framebuffer (wp_project* project, const unsigned int framebuffer) {
	WPENGINE_PROJECT_API_BEGIN
