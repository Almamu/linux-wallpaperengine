...
wp_context* WPENGINE_API wp_context_create (const wp_configuration* config) {
...
void WPENGINE_API wp_context_destroy (wp_context* context) {
	WPENGINE_CONTEXT_API_BEGIN
	delete static_cast<WallpaperEngine::Context*> (context);
	WPENGINE_CONTEXT_API_END ();
}

void WPENGINE_API wp_context_set_gl_proc_address (wp_context* context, wp_gl_proc_address* address) {
	WPENGINE_CONTEXT_API_BEGIN
	static_cast<WallpaperEngine::Context*> (context)->gl_proc_address = address;
	WPENGINE_CONTEXT_API_END ();
}

void WPENGINE_API wp_context_set_time_counter (wp_context* context, wp_time_counter* counter) {
	WPENGINE_CONTEXT_API_BEGIN
