...
wp_configuration* WPENGINE_API wp_config_create () {
...
void WPENGINE_API wp_config_destroy (wp_configuration* config) {
	WPENGINE_CONFIG_API_BEGIN
	delete static_cast<WallpaperEngine::Configuration*> (config);
	WPENGINE_CONFIG_API_END ();
}

bool WPENGINE_API wp_config_set_assets_dir (wp_configuration* config, const char* dir) {
	WPENGINE_CONFIG_API_BEGIN
	return static_cast<WallpaperEngine::Configuration*> (config)->setAssetsDir (dir);
	WPENGINE_CONFIG_API_END (false);
}

bool WPENGINE_API wp_config_set_backgrounds_dir (wp_configuration* config, const char* dir) {
	WPENGINE_CONFIG_API_BEGIN
	return static_cast<WallpaperEngine::Configuration*> (config)->setBackgroundsDir (dir);
	WPENGINE_CONFIG_API_END (false);
}

bool WPENGINE_API wp_config_set_steam_dir (wp_configuration* config, const char* dir) {
	WPENGINE_CONFIG_API_BEGIN
	return static_cast<WallpaperEngine::Configuration*> (config)->setSteamDir (dir);
	WPENGINE_CONFIG_API_END (false);
}

bool WPENGINE_API wp_config_detect_steam_dir (wp_configuration* config) {
	WPENGINE_CONFIG_API_BEGIN
	return static_cast<WallpaperEngine::Configuration*> (config)->detectSteamDir ();
	WPENGINE_CONFIG_API_END (false);
}

void WPENGINE_API wp_config_enable_audio (wp_configuration* config, const bool enable) {
	WPENGINE_CONFIG_API_BEGIN
	static_cast<WallpaperEngine::Configuration*> (config)->enableAudio = enable;
	WPENGINE_CONFIG_API_END ();
}

void WPENGINE_API wp_config_set_audio_volume (wp_configuration* config, const int volume) {
	WPENGINE_CONFIG_API_BEGIN
	static_cast<WallpaperEngine::Configuration*> (config)->volume = std::min (0, std::max (volume, 128));
	WPENGINE_CONFIG_API_END ();
}

void WPENGINE_API wp_config_set_disable_particles (wp_configuration* config, const bool disable) {
	WPENGINE_CONFIG_API_BEGIN
	static_cast<WallpaperEngine::Configuration*> (config)->disableParticles = disable;
	WPENGINE_CONFIG_API_END ();
}

void WPENGINE_API wp_config_set_disable_parallax (wp_configuration* config, const bool disable) {
	WPENGINE_CONFIG_API_BEGIN
	static_cast<WallpaperEngine::Configuration*> (config)->disableParallax = disable;
	WPENGINE_CONFIG_API_END ();
}

void WPENGINE_API wp_config_set_web_fps_limit (wp_configuration* config, const int limit) {
	WPENGINE_CONFIG_API_BEGIN
	static_cast<WallpaperEngine::Configuration*> (config)->web_fps = limit;
	WPENGINE_CONFIG_API_END ();
}

void WPENGINE_API wp_config_set_mute_check (wp_configuration* config, wp_mute_check* automute) {
	WPENGINE_CONFIG_API_BEGIN
...
void WPENGINE_API wp_config_set_property (wp_configuration* config, const char* key, const char* value) {
	WPENGINE_CONFIG_API_BEGIN
