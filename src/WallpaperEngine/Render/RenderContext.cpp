...
RenderContext::RenderContext (Drivers::VideoDriver& driver, WallpaperApplication& app) :
    m_driver (driver),
    m_app (app),
    m_textureCache (new TextureCache (*this)) {}
...
void RenderContext::setPause (bool newState) {
    for (auto&& wallpaper : this->m_wallpapers)
        wallpaper.second->setPause (newState);
}

Input::InputContext& RenderContext::getInputContext () const {
    return this->m_driver.getInputContext ();
}

const WallpaperApplication& RenderContext::getApp () const {
    return this->m_app;
}

const Drivers::VideoDriver& RenderContext::getDriver () const {
    return this->m_driver;
}

const Drivers::Output::Output& RenderContext::getOutput () const {
