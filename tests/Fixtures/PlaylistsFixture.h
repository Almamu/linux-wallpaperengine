...
inline std::map<PlaylistMode, std::string> modeToValue = {
    { PlaylistMode_Timer, "timer" },       { PlaylistMode_Logon, "logon" }, { PlaylistMode_Daytime, "daytime" },
    { PlaylistMode_Weekday, "dayofweek" }, { PlaylsitMode_Never, "never" },
};
...
struct PlaylistsFixture {
    ...
    void addPlaylist (
	const std::string& name, const PlaylistMode mode, const PlaylistOrder order,
	const PlaylistTransition transition, int delay, int transitiontime,
